/*
  ...
  [FORKED for ESP32-CAM PSRAM support]
  Key changes vs original v2.8:
    - bufferSize 类型: uint16_t → uint32_t
    - 内存分配: malloc/realloc/free → heap_caps_malloc/realloc/free
      优先使用 PSRAM (MALLOC_CAP_SPIRAM)，失败时回退到内部 SRAM
    - 这允许 MQTT_MAX_PACKET_SIZE 超过 65535 字节，从而传输更大的 JPEG 图片
*/
...
static uint8_t* psram_aware_malloc(size_t size) {
    ...
    // PSRAM 不足，回退到内部 SRAM
    Serial.printf("[PubSubClient] PSRAM alloc failed for %u bytes, falling back to internal RAM\n", (unsigned)size);
#endif
...
static uint8_t* psram_aware_realloc(uint8_t* old_ptr, size_t old_size, size_t new_size) {
    ...
    Serial.printf("[PubSubClient] PSRAM realloc failed for %u bytes, falling back to internal RAM\n", (unsigned)new_size);
    // 回退：在内部 SRAM 分配新块并复制数据
    uint8_t* new_ptr = (uint8_t*)malloc(new_size);
    ...
    return new_ptr;
#else
    return (uint8_t*)realloc(old_ptr, new_size);
#endif
}

static void psram_aware_free(uint8_t* ptr) {
#ifdef ESP32
    heap_caps_free(ptr);
#else
    free(ptr);
...
// ============================================================
//  核心修改：使用 PSRAM 分配缓冲区，uint32_t 参数无溢出
// ============================================================
